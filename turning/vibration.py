...
def mirrorDf(df):
    ...
    freqDf = freqDf[(freqDf.Sensor1 >= 0) & (freqDf.Sensor1 < 120)]
    freqArr = freqDf['Sensor1'].to_numpy()
    print(np.sort(freqArr))

    freqDf = freqDf[(freqDf.Sensor2 >= 0) & (freqDf.Sensor2 < 120)]
    freqArr2 = freqDf['Sensor2'].to_numpy()
    print(freqArr2)
    
    freqDf = freqDf[(freqDf.Sensor3 >= 0) & (freqDf.Sensor3 < 120)]
    freqArr3 = freqDf['Sensor3'].to_numpy()
    print(freqArr3)

    freqDf = freqDf[(freqDf.Sensor4 >= 0) & (freqDf.Sensor4 < 120)]
    freqArr4 = freqDf['Sensor4'].to_numpy()
    print(np.sort(freqArr4))
    
    calcFreq(freqArr, 'senor1')    
    calcFreq(freqArr2, 'senor2')
    calcFreq(freqArr3, 'senor3')    
    calcFreq(freqArr4, 'senor4')
...
def calcFreq(dfv, sensorNum):
    ...
    arr = dfv[(dfv.Sensor1 >= 0) & (dfv.Sensor1 < 120)]
    vol = fft(arr)
    N = len(vol)
    n = np.arange(N)
    samplingRate = 1/10
